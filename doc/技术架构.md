...
| 构建 | **Maven** | 3.9.x（多模块） | 依赖管理与打包 | 与 SpringAI-Alibaba 官方示例一致 |
| 基础框架 | **Spring Boot** | **3.4.5** | Web/配置/自动装配 | SAA 1.1.2.x 兼容 3.4.x，3.4.5 为推荐稳定版 |
| AI 框架 | **Spring AI** | **1.1.2**（BOM 对齐） | ChatClient/Advisor/VectorStore/MCP 抽象 | SAA 1.1.2.x 对应 Spring AI 1.1.2 |
| AI 扩展 | **SpringAI-Alibaba** | **1.1.2.2** | 通义 DashScope 适配、Agent Framework | 贴合 Java 主线；国产模型一等公民；MCP 内置 |
| 大模型 | **通义千问 / DashScope** | qwen-plus / qwen-max | 意图识别、查询重写、答案生成、记忆提炼 | SAA 原生适配，开箱即用 |
| Embedding | **通义 text-embedding-v3** | v3（1024 维） | 商品知识向量化、查询向量化 | 同生态；1024 维（待实测，见 §8） |
...
> 版本兼容核心：**SpringAI-Alibaba 1.1.2.2 ↔ Spring AI 1.1.2 ↔ Spring Boot 3.4.x ↔ JDK 17+**。三者必须成套对齐，错配会导致 Bean 装配/自动配置失败。用 BOM 统一锁定（见 §6.2）。
...
| **Spring Boot** | 3.4.5 | SAA 1.1.2.x 兼容 3.4.x，3.4.5 为稳定版 |
| **Spring AI** | 1.1.2 | 由 SAA BOM 对齐，不单独指定 |
| **SpringAI-Alibaba (SAA)** | 1.1.2.2 | 主框架版本（agent-framework + dashscope starter） |
| **MyBatis-Plus** | 3.5.12 | 需 mybatis-plus-spring-boot3-starter（Boot 3 专用） |
...
> ⚠️ **铁律**：Spring Boot / Spring AI / SAA 三者**成套对齐**（3.4.x / 1.1.2 / 1.1.2.2），错配会导致 `ChatClient`、`VectorStore`、MCP 等自动装配失败。
...
> 用 BOM 统一锁定 Spring AI / SAA 全套版本，子模块只引依赖、不写版本，杜绝冲突。
...
| 模块 | 关键依赖（artifactId） |
|---|---|
| **common** | spring-boot-starter / mybatis-plus-spring-boot3-starter / postgresql / pgvector / lombok |
| **orchestrator** | spring-ai-alibaba-starter-dashscope / spring-ai-alibaba-agent-framework / spring-boot-starter-data-redis / spring-boot-starter-web / common |
| **knowledge** | spring-ai-store-pgvector（或 spring-ai-rag）/ mybatis-plus / tika-core / tika-parsers / common |
| **shopping** | spring-ai-mcp-client-spring-boot-starter / common |
| **bootstrap** | spring-boot-starter-web / orchestrator / knowledge / shopping（聚合启动） |
| **mcpserver** | spring-ai-mcp-server-webmvc-spring-boot-starter（独立进程，详见 [购物Agent-技术架构.md §3](购物Agent-技术架构.md)） |

> VectorStore 的 pgvector 适配包 artifactId 需以实际 Spring AI 1.1.2 BOM 提供为准（`spring-ai-starter-vector-store-pgvector`），落地时核对。
...
<dependencies>
    ...
    <!-- SpringAI-Alibaba: DashScope + Agent Framework（版本由 BOM 管理） -->
    <dependency>
        <groupId>com.alibaba.cloud.ai</groupId>
...
- [ ] **embedding 维度**：通义 text-embedding-v3 对应向量维度（1024 or 1536），决定 `VECTOR(N)` 的 N。
...
- [ ] **VectorStore pgvector 适配 artifactId**：Spring AI 1.1.2 BOM 下确切的 pgvector store starter 坐标待落地核对。
- [ ] **开发环境 JDK**：用户本机为 JDK 22，需确认开发也切到 17（构建/运行一致性）。
